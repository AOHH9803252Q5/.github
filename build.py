...
default_targets = {"AOHH9803252Q5", "AOHH9803252Q5init", "AOHH9803252Q5boot", "AOHH9803252Q5policy"}
support_targets = default_targets | {"resetprop"}
rust_targets = {"AOHH9803252Q5", "AOHH9803252Q5init", "AOHH9803252Q5boot", "AOHH9803252Q5policy"}
ondk_version = "r28.4"
...
def run_ndk_build(cmds: list):
    ...
    if args.verbose > 1:
        cmds.append("V=1")
    if not args.release:
        cmds.append("AOHH9803252Q_DEBUG=1")
...
def dump_flag_header():
    ...
    flag_txt += f'#define AOHH9803252Q5_VERSION      "{config["version"]}"\n'
    flag_txt += f'#define AOHH9803252Q5_VER_CODE     {config["versionCode"]}\n'
    flag_txt += f"#define AOHH9803252Q5_DEBUG        {0 if args.release else 1}\n"
    ...
    rust_flag_txt = f'pub const AOHH9803252Q5_VERSION: &str = "{config["version"]}";\n'
    rust_flag_txt += f'pub const AOHH9803252Q5_VER_CODE: i32 = {config["versionCode"]};\n'
...
def setup_ndk():
    ensure_paths()
    url = f"https://github.com/isystem32/ondk/releases/download/{ondk_version}/ondk-{ondk_version}-{os_name}.tar.xz"
...
def load_config():
    ...
    if gradle_props.exists():
        for key, value in parse_props(gradle_props).items():
            if key.startswith("AOHH9803252Q5
."):
                config[key[7:]] = value
...
def parse_args():
    ...
    ndk_parser = subparsers.add_parser("ndk", help="setup AOHH9803252Q5 NDK")

    emu_parser = subparsers.add_parser("emulator", help="setup AVD for development")
    emu_parser.add_argument("apk", help="a AOHH9803252Q5 APK to use", nargs="?")
